...
def main():
    ...
    # masks analysis 
    path_masks = '/LUNA_dataset/masks_nodules/nifti_data/'
    ...
    checkpoint = '/projects/repos/MedSAM2/checkpoints/MedSAM2_latest.pt'
    ...
    list_dsc_val_point_ch0 = []
    list_dsc_val_point_ch1 = []
    list_dsc_val_point_ch2 = []
    
    list_dsc_val_boxes_ch0 = []
    list_dsc_val_boxes_ch1 = []
    list_dsc_val_boxes_ch2 = []
    
    list_dsc_val_combined_ch0 = []
    list_dsc_val_combined_ch1 = []
    list_dsc_val_combined_ch2 = []
    
    file_names_list = []   

    for mhd_file in tqdm(only_name_files):
        
        result = analyze_CT_volume_medsam2_image_multi_output(
                            mhd_file=mhd_file,
        ...
        if result is not None: 
            (dsc_point, dsc_box, dsc_combined, *_ ) = result
    
            # ---- store per channel ----
            list_dsc_val_point_ch0.append(dsc_point[0])
            list_dsc_val_point_ch1.append(dsc_point[1])
            list_dsc_val_point_ch2.append(dsc_point[2])
        
            list_dsc_val_boxes_ch0.append(dsc_box[0])
            list_dsc_val_boxes_ch1.append(dsc_box[1])
            list_dsc_val_boxes_ch2.append(dsc_box[2])
        
            list_dsc_val_combined_ch0.append(dsc_combined[0])
            list_dsc_val_combined_ch1.append(dsc_combined[1])
            list_dsc_val_combined_ch2.append(dsc_combined[2])
        
            file_names_list.append(mhd_file)

    df_results_mult_output = pd.DataFrame({
        'VolumeID': file_names_list,
    
        'DSC (points) Ch:0': list_dsc_val_point_ch0,
        'DSC (points) Ch:1': list_dsc_val_point_ch1,
        'DSC (points) Ch:2': list_dsc_val_point_ch2,
    
        'DSC (boxes) Ch:0': list_dsc_val_boxes_ch0,
        'DSC (boxes) Ch:1': list_dsc_val_boxes_ch1,
        'DSC (boxes) Ch:2': list_dsc_val_boxes_ch2,
    
        'DSC (combined) Ch:0': list_dsc_val_combined_ch0,
        'DSC (combined) Ch:1': list_dsc_val_combined_ch1,
        'DSC (combined) Ch:2': list_dsc_val_combined_ch2,
    })
    
    path_save_results = os.path.join(path_dataset, "predictions_MedSAM2_img_multimask_results_LUNA16.csv")
    df_results_mult_output.to_csv(path_save_results, index=False)
    df_results_mult_output.to_csv(path_save_results, index=False)
